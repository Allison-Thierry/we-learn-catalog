...
const faqItems: [string, React.ReactNode][] = [
  ["How do I request a lesson for my country?", "Before a lesson can be assigned, it must be available in the required language or languages. Open the lesson, choose Request assignment and email Allison. In your message, specify the country and the job roles that should receive the lesson. You can also request a Mandatory tag and set a minimum number of completed inventories before the lesson is assigned. If you do not specify these options, the lesson will not be marked as Mandatory and the minimum will be set to zero."],
  ["What if the lesson is not translated yet?", "Translations are handled by pre-selected local champions. Email Allison to check where the lesson sits in the translation pipeline and whether the translation can be started. The initial translation is created with an internal AI tool, then reviewed and corrected by a local champion before publication. This quality check can take a little time while everything is being finalised."],
  ...
  ["How often is the catalog updated?", "The catalog automatically checks the source Google Sheet and refreshes its data every six hours."],
  ["Why can't I see every lesson on WeLearn?", "Lessons are not necessarily assigned to everyone, and what you can access depends on your permissions. If you are an LMS Content Creator, you can assign lessons — including to yourself — directly from the Courses menu in Sheffield. If you do not have the required access, contact your country's LMS Champion or Allison."],
  ["I can't log in to WeLearn. What should I do?", "Your official RGIS credentials and WeLearn credentials are linked: there is no separate WeLearn login. If you only have MyRGIS access, use your MyRGIS credentials. If you have a professional email address ending in @rgis.com, sign in with that email address and your Windows session password."],
  ["I'm stuck in a lesson and can't continue. What should I do?", "Many lessons use locked navigation to make sure every interaction in a module has been completed. Check that you have opened every flip card, answered every quiz question, viewed every slide in each slideshow and completed all the other activities on the page. Once every interaction is complete, you should be able to continue."],
  ["I just love WeLearn so much. How do I express my thoughts?", <span className="faq-love-answer">😊 <a href="mailto:althierry@rgis.com">althierry@rgis.com</a></span>],
];
      ...
      {page === "country" && <section className="section-wrap country-page">
        ...
        {selectedCountry && countryFocus === "overview" && <>
          ...
          <div className="country-insights">
            ...
            <section><div className="insight-heading"><h2>Category overview</h2><p>Assigned lessons by category.</p></div><div className="bar-list">{categoryCounts.slice(0, 6).map(({ category, count, examples }) => <div className="category-bar-row tooltip-anchor" key={category} tabIndex={0} onClick={(event) => event.currentTarget.focus()} aria-label={`${category}: ${count} assigned lesson${count === 1 ? "" : "s"}. Examples: ${examples.join(", ")}.`}><span>{category}</span><div className="category-bar-track"><i style={{ width: `${Math.max(7, Math.round(count / Math.max(...categoryCounts.map((item) => item.count)) * 100))}%` }} /></div><b>{count}</b><div className="hover-tooltip category-tooltip" role="tooltip"><strong>{count} assigned lesson{count === 1 ? "" : "s"}</strong><span>Examples</span><ul>{examples.map((title) => <li key={title}>{title}</li>)}</ul>{count > examples.length && <small>+{count - examples.length} more</small>}</div></div>)}</div></section>
